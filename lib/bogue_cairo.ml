...
module Cairo_area = struct
  ...
  let drawing_size cairo =
    Sdl_area.drawing_size cairo.area
  ...
  let create_cairo_texture renderer w h =
    ...
    Sdl.create_texture renderer format Sdl.Texture.access_streaming ~w ~h
    |> go

  let init_data cairo =
    Bigarray.Array1.fill cairo.data 0
  ...
  let init_cairo c renderer =
    ...
    let target = match Sdl_area.get_texture c.area with
      | None -> failwith "[init_cairo] error: Sdl_area was not initialized."
      | Some t -> t in
    assert ((w,h) =  let _,_,(w,h) = go (Sdl.query_texture target) in w,h);
    let stream = create_cairo_texture renderer w h in
    let data, stream_pitch =
      go @@ Sdl.lock_texture stream None Bigarray.int8_unsigned in
  ...
  let init c =
    Sdl_area.add ~name:"cairo init" c.area (init_cairo c)
  ...
  let set_color cr color =
    ...
    Cairo.set_source_rgba cr (float r /. 255.) (float g /. 255.) (float b /. 255.)
      (float a /. 255.)
  ...
  let draw_rectangle c ?color ~thick ~w ~h (x, y) =
    ...
    add c (fun cr ->
        ...
        set_line_width cr (float thick); (* TODO scale ? *)
