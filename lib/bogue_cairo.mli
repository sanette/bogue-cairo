...
module Cairo_area : sig
  ...
  (** Ask for Cairo init. More precisely, this adds the Cairo init to the SDL
      command queue, and hence will be executed when the area is rendered.
  ...
  (** [add cairo f] adds the command [f] to the Cairo command queue (which is
      ...
      As for Sdl_area, the command should be fast, or else it will block the
      rest of BOGUE's interface. *)
  ...
  (** Clear the whole (SDL+Cairo) command queue (but not the background).
      Warning! this takes effect immediately. This allows to reset the area
      without waiting for the command queue to execute itself. *)
  ...
  val draw_rectangle :
    ...
    int * int ->
    unit
  (** [draw_rectangle cairo ~color ~thick ~w ~h (x0, y0)] draws a rectangle of
      the given line [thick]ness and [color] {e inside} the box of top-left
      coordinates [(x0, y0)], width [w] and height [h]. *)

  val draw_circle :
    t -> ?color:Bogue.Draw.color -> thick:int -> radius:int -> int * int -> unit
  (** [draw_circle cairo ~color ~thick ~radius (x0, y0)] draws a circle of the
      given line [thick]ness and [color] {e inside} the disc of center
      coordinates [(x0, y0)] and given [radius]. *)
end

(** {3 Example}

    If you have installed [Bogue], you may run [boguex 50] to see a demo of
    drawing circles using the native Bogue circle primitive. Here is an
    alternative version, using Cairo's circles, which is twice as fast.
